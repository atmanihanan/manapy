...
from manapy.ast.functions2d import (centertovertex_2d, face_gradient_2d,compute_P_gradient_2d_diamond,
                                    cell_gradient_2d, compute_gradient_2d_FV4,barthlimiter_2d,vanalbadalimiter_2d,minmodFacelimiter_2d,minmodNodelimiter_2d)
...
class Variable():
    ...
    def __init__(self, domain=None, terms=None, comm=None, name=None, BC=None, values=None, *args, **kwargs):
        ...
        self._nballfaces=len(self.domain.innerfaces)+ len(self.domain.halofaces)+len(self.domain.boundaryfaces)
        ...
        #self.psi = np.zeros(self._nbcells, dtype=self.float_precision)
        self.minmod_x= np.zeros(self._nbcells, dtype=self.float_precision)
        self.minmod_halo_x = np.zeros(self._nbhalos, dtype=self.float_precision)
        self.minmod_y= np.zeros(self._nbcells, dtype=self.float_precision)
        self.minmod_halo_y = np.zeros(self._nbhalos, dtype=self.float_precision)

        self.vanalbada_limiter = np.zeros((self._nbfaces, 2), dtype=self.float_precision)
        self.vanalbada_halo_limiter = np.zeros((self._nbfaces, 2), dtype=self.float_precision)
        
        self.van_array_limiter = np.zeros(2*self._nbfaces, dtype=self.float_precision)
        self.van_array_halo_limiter = np.zeros(2*self._nbfaces, dtype=self.float_precision)
    ...
    def compile_func(cls, dim, backend, signature, forcedbackend):
        if not cls.is_called:
            ...
            if dim == 2:
                ...
                #cls._face_gradient = backend.compile(  compute_gradient_2d_FV4, signature=signature)
                cls._face_gradient = backend.compile(compute_P_gradient_2d_diamond, signature=signature)
                ...
                cls._vanalbadalimiter  = backend.compile(vanalbadalimiter_2d, signature=signature)
                cls._minmodlimiter_node  = backend.compile(minmodNodelimiter_2d, signature=signature)
                cls._minmodlimiter_face = backend.compile(minmodFacelimiter_2d, signature=signature)
            elif dim == 3:
...
#        self.comm.Barrier()
    ...
    def compute_cell_gradient(self,limiter):
        ...
        if limiter==3:
            ...
            Variable._vanalbadalimiter(self.cell, self.ghost, self.halo, self.gradcellx, self.gradcelly,self.gradhalocellx,self.gradhalocelly,self.domain.cells.center, self.domain.halos.centvol, self.domain.faces.ghostcenter,self.domain.innerfaces, 
                    self.domain.halofaces, self.domain.boundaryfaces, self.domain.faces.cellid,self.vanalbada_limiter, self.domain.faces.halofid)
            self.van_array_limiter=self.vanalbada_limiter.flatten() 
             
            self.comm.Barrier()
            #update the halo values
            Variable._define_halosend(self.van_array_limiter, self.halotosend, self.domain.halos.indsend)
            all_to_all(self.halotosend, self.nbhalos, self.domain.halos.scount, self.domain.halos.rcount, self.van_array_halo_limiter,
                    self.comm, self.mpi_precision)
            
            self.vanalbada_halo_limiter=self.van_array_halo_limiter.reshape(-1, 2)
            

            
        elif limiter==1:
   
            # Variable._minmodlimiter_face(self.cell, self.gradcellx, self.gradcelly,self.gradhalocellx, self.gradhalocelly, 
            #                 self.domain.cells.faceid, self.domain.faces.cellid, 
            #                 self.domain.faces.halofid, self.domain.innerfaces, self.domain.boundaryfaces ,self.domain.halofaces,self.minmod_x, self.minmod_y)
            ...
            Variable._minmodlimiter_face(self.cell, self.gradcellx, self.gradcelly,self.gradhalocellx, self.gradhalocelly, 
                            self.domain.cells.faceid, self.domain.faces.cellid, 
                            self.domain.faces.halofid, self.minmod_x, self.minmod_y,self.domain.faces.name)
            
        
         
            self.comm.Barrier()
            Variable._define_halosend(self.minmod_x, self.halotosend, self.domain.halos.indsend)
            all_to_all(self.halotosend, self.nbhalos, self.domain.halos.scount, self.domain.halos.rcount, self.minmod_halo_x,
                    self.comm, self.mpi_precision)
            
            Variable._define_halosend(self.minmod_y, self.halotosend, self.domain.halos.indsend)
            all_to_all(self.halotosend, self.nbhalos, self.domain.halos.scount, self.domain.halos.rcount, self.minmod_halo_y,
                    self.comm, self.mpi_precision)
            
        elif limiter==4:
 
            # Variable._minmodlimiter_node(self.cell, self.gradcellx, self.gradcelly, self.gradhalocellx, self.gradhalocelly,
            #                               self.domain.cells.nodeid,
            #                  self.domain.nodes.cellid,self.domain.nodes.ghostid,self.domain.nodes.haloghostid,self.minmod_x, self.minmod_y)
    ...
    def compute_face_gradient(self):
        ...
        #  ##FV4
        #     Variable._face_gradient(self.cell, self.ghost, self.halo, self.domain.faces.cellid, 
        #              self.domain.faces.center,  self.domain.faces.halofid, self.gradfacex, self.gradfacey
        #              ,self.domain.innerfaces,self.dirichletfaces, self.domain.halofaces,self.domain.faces.normal, self.domain.faces._dist_ortho, self.domain.faces.mesure)
     ## face_gradient
        ...
        Variable._face_gradient(self.cell, self.ghost, self.halo, self.node, self.domain.faces.cellid, 
                            self.domain.faces.nodeid, self.domain.faces.ghostcenter, 
                            self.domain.faces.halofid, self.domain.cells.center,
                            self.domain.halos.centvol, self.domain.nodes.oldname, self.domain.faces.airDiamond,self.domain.faces.f_1,self.domain.faces.f_2,self.domain.faces.f_3,
                            self.domain.faces.f_4,self.domain.faces.normal,self.domain.cells.shift,  self.domain.Pbordnode,self.domain.Pbordface,
                            self.gradfacex, self.gradfacey, self.gradfacez,self.BCdirichlet, self.domain.innerfaces,
                            self.domain.halofaces, self.neumannfaces, self.dirichletfaces, 
                            self.domain.periodicboundaryfaces)
