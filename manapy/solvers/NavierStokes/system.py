...
from manapy.solvers.NavierStokes import (update_euler, time_step_euler, explicitscheme_convective_euler,explicitscheme_dissipative_NS,term_source_poiseuille)
...
class EulerSolver():
    ...
    def explicit_dissipative(self):
        ...
        explicitscheme_dissipative_NS(self.rho.cell, self.rhou.cell, self.rhov.cell,self.rhoE.cell, self.P.cell,
                                    self.rho.ghost, self.rhou.ghost, self.rhov.ghost, self.rhoE.ghost, self.P.ghost,
                                    self.rho.halo, self.rhou.halo, self.rhov.halo,self.rhoE.halo,self.P.halo,
                                    self.rho.gradfacex,self.rho.gradfacey,self.rhou.gradfacex,self.rhou.gradfacey,
                                    self.rhov.gradfacex,self.rhov.gradfacey, self.rhoE.gradfacex,self.rhoE.gradfacey,self.P.gradfacex,self.P.gradfacey,
                                    self.rhou.dissipation,self.rhov.dissipation ,self.rhoE.dissipation, 
                                    ...
                                    self.T_ref,self.mu_ref,self.C_zero,self.Cp,self.Pr,self.R,self.gamma, self.mu, self.domain.faces.name)
    ...
    def stepper(self):
        ...
        dt_c = self._time_step_euler(self.rho.cell, self.rhou.cell, self.rhov.cell, self.rhoE.cell,self.P.cell, self.cfl, self.domain.faces.normal, self.domain.faces.mesure, 
                                  self.domain.cells.volume, self.domain.cells.faceid, self.gamma,self.Pr,self.mu)
    ...
    def update_term_source(self):
        term_source_poiseuille(self.rho.source, self.rhou.source, self.rhov.source,self.rhoE.source)
    ...
    def compute_new_val(self):
        ...
        self._update_new_value(self.rho.cell, self.rhou.cell, self.rhov.cell, self.rhoE.cell,self.e_internal.cell,self.P.cell ,
                               self.rho.convective, self.rhou.convective, self.rhov.convective, self.rhoE.convective,self.rhou.dissipation,
                               self.rhov.dissipation ,self.rhoE.dissipation, 
                                self.dt, self.domain.cells.volume, self.gamma,self.rho.source,self.rhou.source,self.rhov.source,self.rhoE.source)
