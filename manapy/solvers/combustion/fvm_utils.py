...
def time_step_euler(rho_c:'float[:]', rhou_c:'float[:]', rhov_c:'float[:]',rhoE_c:'float[:]',P_c:'float[:]', cfl:'float', normal:'float[:,:]', mesure:'float[:]', volume:'float[:]', 
   ...
   # dt= max(dt,0.00001)
    return dt
'''
def time_step_euler(rho_c:'float[:]', rhou_c:'float[:]', rhov_c:'float[:]',T_c:'float[:]',P_c:'float[:]', cfl:'float', normal:'float[:,:]', mesure:'float[:]', volume:'float[:]', 
                 faceid:'int32[:,:]', gamma:'float',Pr:'float', Sc1:'float', Sc2:'float', T_ref:'float', mu_ref:'float',C_zero:'float'):
    nbelement =  len(faceid)
    u_n = 0.
    dt_c=np.zeros(nbelement)
    norm = zeros(3)
    ...
    for i in range(nbelement):
        lam = 0.
        rho_c_safe = max(rho_c[i], 1e-12)
        T_c_safe = max(T_c[i], 1e-12)
        c = np.sqrt(gamma*np.abs(P_c[i]/rho_c_safe ))
        
        mu=mu_ref*np.sqrt(T_c[i]/T_ref)*(1+C_zero/T_ref)/(1+C_zero/T_c_safe)
        D1=mu/(Sc1*rho_c_safe)
        D2=mu/(Sc2*rho_c_safe)
        ...
        for j in range(faceid[i][-1]):
            ...
            # Dxx=mu
            # Dyy=mu
            Dxx=max(4/3*mu,mu, 4/3*mu*u,v*mu,mu*gamma/Pr,D1,D2)
            Dyy=max(4/3*mu,mu, 4/3*mu*v,u*mu,mu*gamma/Pr,D1,D2)
...
def update_euler(rho_c:'float[:]', rhou_c:'float[:]', rhov_c:'float[:]', rhoE_c:'float[:]',e_interanl_c:'float[:]', P_c:'float [:]',T_c:'float[:]',rhoY1_c:'float[:]',rhoY2_c:'float[:]',Y1_c:'float[:]',Y2_c:'float[:]',
              rez_rho:'float[:]', rez_rhou:'float[:]', rez_rhov:'float[:]', rez_rhoE:'float[:]',rez_rhoY1:'float[:]',rez_rhoY2:'float[:]',
              diss_rhou:'float[:]',diss_rhov:'float[:]',diss_rhoE:'float[:]',diss_rhoY1:'float[:]',diss_rhoY2:'float[:]',
              dtime:'float', vol:'float[:]',gamma:'float', src_rho:'float[:]', src_rhou:'float[:]', src_rhov:'float[:]', src_rhoE:'float[:]',src_rhoY1:'float[:]',src_rhoY2:'float[:]',  R:'float'):

    for i in range(len(rho_c)):
        rho_c[i]   = rho_c[i] + dtime*(rez_rho[i])/vol[i] + dtime*src_rho[i]
        rhou_c[i]  = rhou_c[i] + dtime*(rez_rhou[i]-diss_rhou[i])/vol[i] + dtime*src_rhou[i]
        rhov_c[i]  = rhov_c[i] + dtime*(rez_rhov[i]-diss_rhov[i])/vol[i] + dtime*src_rhov[i]
        rhoE_c[i]  = rhoE_c[i] + dtime*(rez_rhoE[i]-diss_rhoE[i])/vol[i] + dtime*src_rhoE[i]
        rhoY1_c[i]  = rhoY1_c[i] + dtime*(rez_rhoY1[i]-diss_rhoY1[i])/vol[i] + dtime*src_rhoY1[i]
        rhoY2_c[i]  = rhoY2_c[i] + dtime*(rez_rhoY2[i]-diss_rhoY2[i])/vol[i] + dtime*src_rhoY2[i]
 
        P_c[i]= (gamma - 1) * (rhoE_c[i] - 0.5 * (rhou_c[i]**2 + rhov_c[i]**2) / rho_c[i])
        e_interanl_c[i]=P_c[i] / (rho_c[i] * (gamma - 1.))
        Y1_c[i]= rhoY1_c[i] /rho_c[i]
        Y2_c[i]= rhoY2_c[i] /rho_c[i]          
        T_c[i]=P_c[i]/(rho_c[i]*R)



def term_source(src_rho:'float[:]', src_rhou:'float[:]', src_rhov:'float[:]', src_rhoE:'float[:]',src_rhoY1:'float[:]',src_rhoY2:'float[:]',T:'float[:]', rhoY1:'float[:]', rhoY2:'float[:]', B:'float', Ta:'float',alpha:'float',deltaQ:'float',M1:'float',M2:'float', mu1:'int32',mu2:'int32',nu1:'int32', nu2:'int32'):   
    nbelement = len(src_rho)
    for i in range(nbelement): 
        # print(M1, M2,mu1,mu2,nu1,nu2, B,alpha,deltaQ,Ta,T[i] ) 
        pro=((rhoY1[i]/M1)**mu1)*((rhoY2[i]/M2)**mu2)
        w=B*(T[i]**alpha)*np.exp(-Ta/T[i])*pro
        # if np.abs(w)>1e-6:
        #   print("w=",w)
        src_rho[i]   = 0.0
        src_rhou[i]  = 0.0
        src_rhov[i]  = 0.0
        src_rhoE[i]  = w*deltaQ 
        src_rhoY1[i]  = M1*(nu1-mu1)*w
        src_rhoY2[i]  =  M2*(nu2-mu2)*w


def explicitscheme_dissipative_NS(rho_c:'float[:]', rhou_c:'float[:]', rhov_c:'float[:]',  rhoE_c:'float[:]',P_c:'float[:]',T_c:'float[:]',Y1_c:'float[:]',Y2_c:'float[:]',
                               rho_ghost:'float[:]', rhou_ghost:'float[:]', rhov_ghost:'float[:]', rhoE_ghost:'float[:]',P_ghost:'float[:]',T_ghost:'float[:]',Y1_ghost:'float[:]',Y2_ghost:'float[:]',
                               rho_halo:'float[:]', rhou_halo:'float[:]', rhov_halo:'float[:]',rhoE_halo:'float[:]',P_halo:'float[:]',T_halo:'float[:]', Y1_halo:'float[:]',Y2_halo:'float[:]',rhox_face:'float[:]',rhoy_face:'float[:]',  rhoux_face:'float[:]',rhouy_face:'float[:]', rhovx_face:'float[:]',
                                rhovy_face:'float[:]',rhoEx_face:'float[:]',rhoEy_face:'float[:]',Px_face:'float[:]',Py_face:'float[:]',Tx_face:'float[:]',Ty_face:'float[:]', Y1x_face:'float[:]',Y1y_face:'float[:]',Y2x_face:'float[:]',Y2y_face:'float[:]',
                                dissip_rhou:'float[:]', dissip_rhov:'float[:]', dissip_rhoE:'float[:]',dissip_rhoY1:'float[:]',dissip_rhoY2:'float[:]',
                               innerfaces:'int32[:]', halofaces:'int32[:]', boundaryfaces:'int32[:]',normalf:'float[:,:]', mesuref:'float[:]', cellidf:'int32[:,:]',halofid:'int32[:]',
                                T_ref:'float', mu_ref:'float',C_zero:'float',Cp:'float', Pr:'float',R:'int32',gamma:'float',namef:'uint32[:]',Sc1:'float',Sc2:'float' ):
    
    dissip_rhou[:] = 0.; dissip_rhov[:] = 0.; dissip_rhoE[:] = 0.;dissip_rhoY1[:] = 0.;dissip_rhoY2[:] = 0.
    # print("je suis ici")
    for i in innerfaces:
      ...
      Y1_l=Y1_c[cellidf[i][0]]
      Y2_l=Y2_c[cellidf[i][0]]

      rho_r  = rho_c[cellidf[i][1]]
      rhou_r = rhou_c[cellidf[i][1]]
      rhov_r = rhov_c[cellidf[i][1]]
      P_r    = P_c[cellidf[i][1]]
      Y1_r    = Y1_c[cellidf[i][1]]
      Y2_r    = Y2_c[cellidf[i][1]]
      T_r=T_c[cellidf[i][1]]
      ...
      tau_xy=mu_f*(u_y+v_x)
      # print("lamda",Cp/(Pr)*mu_f)
      q_x=-Cp/(Pr)*mu_f*Tx_face[i]
      q_y=-Cp/(Pr)*mu_f*Ty_face[i]
      ...
      diss_rhoY1=D1*Y1x_face[i]*nx+D1*Y1y_face[i]*ny
      diss_rhoY2=D2*Y2x_face[i]*nx+D2*Y2y_face[i]*ny
    
    
      dissip_rhou[cellidf[i][0]] -= diss_rhou
      dissip_rhov[cellidf[i][0]] -= diss_rhov
      dissip_rhoE[cellidf[i][0]] -= diss_rhoE
      dissip_rhoY1[cellidf[i][0]] -= diss_rhoY1
      dissip_rhoY2[cellidf[i][0]] -= diss_rhoY2
      
      dissip_rhou[cellidf[i][1]] += diss_rhou
      dissip_rhov[cellidf[i][1]] +=diss_rhov
      dissip_rhoE[cellidf[i][1]] +=diss_rhoE
      dissip_rhoY1[cellidf[i][1]] +=diss_rhoY1
      dissip_rhoY2[cellidf[i][1]] +=diss_rhoY2
    ...
    for i in halofaces :
      ...
      T_r = T_halo[halofid[i]]
      Y1_r = Y1_halo[halofid[i]]
      Y2_r = Y2_halo[halofid[i]]
...
def explicitscheme_convective_euler(rez_rho:'float[:]', rez_rhou:'float[:]', rez_rhov:'float[:]', rez_rhoE:'float[:]', rez_rhoY1:'float[:]',rez_rhoY2:'float[:]',
                                 rho_c:'float[:]', rhou_c:'float[:]', rhov_c:'float[:]', rhoE_c:'float[:]', P_c:'float[:]',rhoY1_c:'float[:]',rhoY2_c:'float[:]',
                                 rho_ghost:'float[:]', rhou_ghost:'float[:]', rhov_ghost:'float[:]', rhoE_ghost:'float[:]',P_ghost:'float[:]',rhoY1_ghost:'float[:]',rhoY2_ghost:'float[:]',
                                 rho_halo:'float[:]', rhou_halo:'float[:]', rhov_halo:'float[:]', rhoE_halo:'float[:]',P_halo:'float[:]',rhoY1_halo:'float[:]',rhoY2_halo:'float[:]',
                                 ...
                                 P_x:'float[:]', P_y:'float[:]', Px_halo:'float[:]', Py_halo:'float[:]',rhoY1_x:'float[:]', rhoY1_y:'float[:]', rhoY1x_halo:'float[:]', rhoY1y_halo:'float[:]',rhoY2_x:'float[:]',rhoY2_y:'float[:]', rhoY2x_halo:'float[:]', rhoY2y_halo:'float[:]',rho_psi:'float[:]',psirho_halo:'float[:]',rhou_psi:'float[:]',
                                 psirhou_halo:'float[:]',rhov_psi:'float[:]',psirhov_halo:'float[:]',rhoE_psi:'float[:]',psirhoE_halo:'float[:]',P_psi:'float[:]',psiP_halo:'float[:]',rhoY1_psi:'float[:]',psirhoY1_halo:'float[:]',rhoY2_psi:'float[:]',psirhoY2_halo:'float[:]',
                                 ...
                                 ,limiter: 'int32',minmodrho_x:'float[:]',minmodrho_y:'float[:]',minmodrho_halo_x:'float[:]'
                                  ...
                                  ,minmodP_y:'float[:]',minmodP_halo_x:'float[:]',minmodP_halo_y:'float[:]',minmodY1_x:'float[:]'
                                  ,minmodY1_y:'float[:]',minmodY1_halo_x:'float[:]',minmodY1_halo_y:'float[:]',minmodY2_x:'float[:]'
                                  ,minmodY2_y:'float[:]',minmodY2_halo_x:'float[:]',minmodY2_halo_y:'float[:]',
                                  vanalbadarho_limiter:'float[:,:]',vanalbadarhou_limiter:'float[:,:]',vanalbadarhov_limiter:'float[:,:]',
                                  vanalbadarhoE_limiter:'float[:,:]',vanalbadaP_limiter:'float[:,:]',vanalbadarho_halo:'float[:,:]',vanalbadarhou_halo:'float[:,:]',vanalbadarhov_halo:'float[:,:]',vanalbadarhoE_halo:'float[:,:]',vanalbadaP_halo:'float[:,:]',vanalbadaY1_limiter:'float[:,:]',vanalbadaY2_limiter:'float[:,:]',vanalbadaY1_halo:'float[:,:]',vanalbadaY2_halo:'float[:,:]'  ):
  
    

            
            #print("le nombre d'element au bord:",boundaryfaces)barthlimiter_2d
            rez_rho[:] = 0.; rez_rhou[:] = 0.; rez_rhov[:] = 0.; rez_rhoE[:] = 0.; rez_rhoY1[:] = 0.;rez_rhoY2[:] = 0.
            ...
            for i in innerfaces:
                ...
                # Y1_l    = Y1_c[cellidf[i][0]]
                # Y2_l    = Y2_c[cellidf[i][0]]
                rhoY1_l    = rhoY1_c[cellidf[i][0]]
                rhoY2_l    = rhoY2_c[cellidf[i][0]]
                ...
                # Y1_r   = Y1_c[cellidf[i][1]]
                # Y2_r   = Y2_c[cellidf[i][1]]
                rhoY1_r   = rhoY1_c[cellidf[i][1]]
                rhoY2_r  = rhoY2_c[cellidf[i][1]]
              ...
              # rhoE
                ...
                #rhoY1
                rhoY1_x_left = rhoY1_x[cellidf[i][0]] 
                rhoY1_y_left = rhoY1_y[cellidf[i][0]]
                rhoY1_x_right = rhoY1_x[cellidf[i][1]]
                rhoY1_y_right = rhoY1_y[cellidf[i][1]]


                   #rhoY1
                rhoY2_x_left = rhoY2_x[cellidf[i][0]] 
                rhoY2_y_left = rhoY2_y[cellidf[i][0]]
                rhoY2_x_right = rhoY2_x[cellidf[i][1]]
                rhoY2_y_right = rhoY2_y[cellidf[i][1]]
                ...
                if order==2:     
                  if limiter==3:
                    ...
                    rhoY1_l =  rhoY1_l +rhoY1_psi[cellidf[i][0]]*( rhoY1_x_left * r_l[0]  +  rhoY1_y_left * r_l[1]  )
                    rhoY1_r =  rhoY1_r+rhoY1_psi[cellidf[i][1]]*( rhoY1_x_right * r_r[0]  +  rhoY1_y_right * r_r[1]  )

                    rhoY2_l =  rhoY2_l +rhoY1_psi[cellidf[i][0]]*( rhoY2_x_left * r_l[0]  +  rhoY2_y_left * r_l[1]  )
                    rhoY2_r =  rhoY2_r+rhoY1_psi[cellidf[i][1]]*( rhoY2_x_right * r_r[0]  +  rhoY2_y_right * r_r[1]  )
                    ...
                    rhoY1_l=  rhoY1_l +0.5*vanalbadaY1_limiter[i][0]
                    rhoY1_r= rhoY1_r -0.5*vanalbadaY1_limiter[i][1]

                    rhoY2_l=  rhoY2_l +0.5*vanalbadaY2_limiter[i][0]
                    rhoY2_r= rhoY2_r -0.5*vanalbadaY2_limiter[i][1]
                    ...
                    rhoY1_l =  rhoY1_l +(minmodY1_x[cellidf[i][0]]*r_l[0]+minmodY1_y[cellidf[i][0]]*r_l[1])
                    rhoY1_r =  rhoY1_r +(minmodY1_x[cellidf[i][1]]*r_r[0]+minmodY1_y[cellidf[i][1]]*r_r[1])

                    rhoY2_l =  rhoY2_l +(minmodY2_x[cellidf[i][0]]*r_l[0]+minmodY2_y[cellidf[i][0]]*r_l[1])
                    rhoY2_r =  rhoY2_r +(minmodY2_x[cellidf[i][1]]*r_r[0]+minmodY2_y[cellidf[i][1]]*r_r[1])
              ...
              ################ available schemes################""
                if scheme_to_exec == 1:
                    ...
                    Roe_version2(rho_l,rhou_l,rhov_l, rhoE_l,P_l,rhoY1_l, rhoY2_l,rho_r,rhou_r, rhov_r,rhoE_r,P_r ,rhoY1_r, rhoY2_r,gamma, normal, tangent,flux,mesure)
                elif scheme_to_exec == 3: #"HLL"
                    HLL_euler_2d(rho_l,rhou_l,rhov_l, rhoE_l,P_l,rho_r,rhou_r, rhov_r,rhoE_r,P_r ,rhoY1_r,rhoY1_l,rhoY2_r,rhoY2_l,gamma, normal, tangent,flux,mesure)
                ...
                rez_rhoY1[cellidf[i][0]] -= flux[4]
                rez_rhoY2[cellidf[i][0]] -= flux[5]
                
                rez_rho[cellidf[i][1]]  += flux[0]
                rez_rhou[cellidf[i][1]] += flux[1]
                rez_rhov[cellidf[i][1]] += flux[2]
                rez_rhoE[cellidf[i][1]] += flux[3]
                rez_rhoY1[cellidf[i][1]] += flux[4]
                rez_rhoY2[cellidf[i][1]] += flux[5]
            ...
            for i in halofaces :
                ...
                rhoE_r = rhoE_halo[halofid[i]]
                P_r = P_halo[halofid[i]]
                rhoY1_r = rhoY1_halo[halofid[i]]
                rhoY2_r = rhoY2_halo[halofid[i]]
              ...
              # rhoE
                ...
                rhoY1_x_left = rhoY1_x[cellidf[i][0]];  rhoY1_x_right = rhoY1x_halo[halofid[i]]
                rhoY1_y_left = rhoY1_y[cellidf[i][0]];  rhoY1_y_right = rhoY1y_halo[halofid[i]]
                             # p
                rhoY2_x_left =  rhoY2_x[cellidf[i][0]];   rhoY2_x_right =  rhoY2x_halo[halofid[i]]
                rhoY2_y_left =  rhoY2_y[cellidf[i][0]];   rhoY2_y_right =  rhoY2y_halo[halofid[i]]
                ...
                if order==2:
                  if limiter==3:
                    ...
                    rhoY1_l =rhoY1_l +rhoY1_psi[cellidf[i][0]]*(rhoY1_x_left * r_l[0]  + rhoY1_y_left * r_l[1]  )
                    rhoY1_r = rhoY1_r+psirhoY1_halo[halofid[i]]*(rhoY1_x_right * r_r[0]  + rhoY1_y_right* r_r[1]  )
                    rhoY2_l = rhoY2_l +rhoY2_psi[cellidf[i][0]]*(rhoY2_x_left * r_l[0]  + rhoY2_y_left * r_l[1]  )
                    rhoY2_r = rhoY2_r+psirhoY2_halo[halofid[i]]*(rhoY2_x_right * r_r[0]  + rhoY2_y_right* r_r[1]  )
                  ...
                  #####Vanalbada
                    ...
                    rhoY1_l= rhoY1_l +0.5*vanalbadaY1_limiter[i][0]
                    rhoY1_r= rhoY1_r -0.5*vanalbadaY1_halo[i][1]
                    rhoY2_l= rhoY2_l +0.5*vanalbadaY2_limiter[i][0]
                    rhoY2_r= rhoY2_r -0.5*vanalbadaY2_halo[i][1]
                    ...
                    rhoY1_l =rhoY1_l +(minmodY1_x[cellidf[i][0]]*r_l[0]+minmodY1_y[cellidf[i][0]]*r_l[1])
                    rhoY1_r =rhoY1_r +(minmodY1_halo_x[halofid[i]]*r_r[0]+minmodY1_halo_y[halofid[i],]*r_r[1])
                   
                    rhoY2_l =rhoY2_l +(minmodY2_x[cellidf[i][0]]*r_l[0]+minmodY2_y[cellidf[i][0]]*r_l[1])
                    rhoY2_r =rhoY2_r +(minmodY2_halo_x[halofid[i]]*r_r[0]+minmodY2_halo_y[halofid[i],]*r_r[1])
  ...
  #######################" available schemes#####################""
            ...
            for i in boundaryfaces:
                ...
                rhoY1_r = rhoY1_ghost[i]
                rhoY2_r = rhoY2_ghost[i]
...
@njit
def Roe_version2(rho_l:'float',rhou_l:'float',rhov_l:'float',rhoE_l:'float',P_l:'float', rhoY1_l:'float', rhoY2_l:'float',rho_r:'float',rhou_r:'float',rhov_r:'float',rhoE_r:'float',P_r:'float',rhoY1_r:'float', rhoY2_r:'float', gamma:'float', normal:'float[:]',tangent:'float[:]',flux:'float[:]',mesure:'float'):
  ...
  #initialisation of matrix
    ...
    Htilde=(np.sqrt(rho_l)*H_l+np.sqrt(rho_r)*H_r)/d
    # ptilde=(np.sqrt(rho_l)*P_l+np.sqrt(rho_r)*P_r)/d
    Y1tilde=(np.sqrt(rho_l)*Y1_l+np.sqrt(rho_r)*Y1_r)/d
    Y2tilde=(np.sqrt(rho_l)*Y2_l+np.sqrt(rho_r)*Y2_r)/d
    #ctilde=np.sqrt(gamma*ptilde/rhotilde)
  ...
  # corrction entropique
   ...
   ### F1(w_left)
    ...
    F1_rhov_l= rhou_l*v_l
    F1_rhoE_l= (rhoE_l+P_l)*u_l
    F1_rhoY1_l=rhou_l*Y1_l
    F1_rhoY2_l=rhou_l*Y2_l
  ...
  ### F1(w_right)
    ...
    F1_rhov_r=rhou_r*v_r
    F1_rhoE_r=(rhoE_r+P_r)*u_r
    F1_rhoY1_r=rhou_r*Y1_r
    F1_rhoY2_r=rhou_r*Y2_r
  ...
  ### F2(w_left)
    ...
    F2_rhov_l= rhov_l*v_l+P_l
    F2_rhoE_l= (rhoE_l+P_l)*v_l
    F2_rhoY1_l=rhov_l*Y1_l
    F2_rhoY2_l=rhov_l*Y2_l
  ...
  ### F2(w_right)
    ...
    F2_rhov_r=rhov_r*v_r+P_r
    F2_rhoE_r= (rhoE_r+P_r)*v_r
    F2_rhoY1_r=rhov_r*Y1_r
    F2_rhoY2_r=rhov_r*Y2_r

    W_right_left=np.array([rho_r-rho_l, rhou_r-rhou_l, rhov_r-rhov_l, rhoE_r-rhoE_l, rhoY1_r-rhoY1_l, rhoY2_r-rhoY2_l])
    
    R1[0][0]=1/ctilde
    R1[1][0]=utilde/ctilde -nx
    R1[2][0]=vtilde/ctilde -ny
    R1[3][0]=(vtilde**2+utilde**2)/(2*ctilde) - vtilde*ny-utilde*nx + ctilde/(gamma-1)
    R1[4][0]=Y1tilde/ctilde
    R1[5][0]=Y2tilde/ctilde
    ...
    R4[4][0]=Y1tilde/ctilde
    R4[5][0]=Y2tilde/ctilde
    ...
    if lamd2<0:
      if lamd4<=0:
        ...
        flux[4]=(F1_rhoY1_r*nx+ F2_rhoY1_r*ny)*mesure
        flux[5]=(F1_rhoY2_r*nx+ F2_rhoY2_r*ny)*mesure
        ...
        flux[4]=(F1_rhoY1_r*nx+ F2_rhoY1_r*ny-lamd4*diff2[4])*mesure
        flux[5]=(F1_rhoY2_r*nx+ F2_rhoY2_r*ny-lamd4*diff2[5])*mesure
    else:
      if lamd1>=0:
        ...
        flux[4]=(F1_rhoY1_l*nx+ F2_rhoY1_l*ny)*mesure
        flux[5]=(F1_rhoY2_l*nx+ F2_rhoY2_l*ny)*mesure
      else:
        ...
        flux[4]=(F1_rhoY1_l*nx+ F2_rhoY1_l*ny+ lamd1*diff1[4])*mesure
        flux[5]=(F1_rhoY2_l*nx+ F2_rhoY2_l*ny+ lamd1*diff1[5])*mesure
...
@njit
def HLL_euler_2d(rho_l:'float',rhou_l:'float',rhov_l:'float',rhoE_l:'float',P_l:'float',rho_r:'float',rhou_r:'float',rhov_r:'float',rhoE_r:'float',P_r:'float',rhoY1_r:'float',rhoY1_l:'float',rhoY2_r:'float',rhoY2_l:'float', gamma:'float', normal:'float[:]',tangent:'float[:]',flux:'float[:]',mesure:'float'):
  ...
  ### F2(w_right)
    ...
    if SL>0:
      ...
      flux[4]=(((F1_rhoY1_l*nx + F2_rhoY1_l*ny)*SR-SL*(F1_rhoY1_r*nx+ F2_rhoY1_r*ny)+SL*SR*(rhoY1_r-rhoY1_l))/(SR-SL))*mesure
      flux[5]=(((F1_rhoY2_l*nx + F2_rhoY2_l*ny)*SR-SL*(F1_rhoY2_r*nx+ F2_rhoY2_r*ny)+SL*SR*(rhoY2_r-rhoY2_l))/(SR-SL))*mesure
