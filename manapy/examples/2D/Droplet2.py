...
from manapy.solvers.combustion.tools_utils import initialisation_Droplet, ghost_value_Droplet2,haloghost_value_Droplet2
...
while time < tfinal:
    ...
    ghost_value_Droplet2(rho.ghost,P.ghost,rhou.ghost,rhov.ghost,
...
#cell
    haloghost_value_Droplet2(rho.halo,rhou.halo, rhov.halo, rhoE.halo, P.halo, T.halo,rhoY1.halo,rhoY2.halo, Y1.halo,Y2.halo, 
...
if RANK == 0:
    ...
    def plot_isovaleurs(x, y, values, title, filename, cmap="seismic", label="Variable", nb=6, mi=0, ma=1):
        ...
        # 🔹 Créer la triangulation complète
        triang = Triangulation(x, y)

        # 🔹 Calcul des barycentres des triangles
        xtri = x[triang.triangles].mean(axis=1)
        ytri = y[triang.triangles].mean(axis=1)

        # 🔹 Définition de la cavité à masquer (demi-bulle)
        x_c, y_c = 0.0, 0.05   # centre de la cavité
        r = 0.01               # rayon
        inside_circle = ((xtri - x_c)**2 + (ytri - y_c)**2) < r**2

        # 🔹 Appliquer le masque aux triangles
        triang.set_mask(inside_circle)
        ...
        cbar = plt.colorbar(tpc, cax=cax, orientation="vertical")
