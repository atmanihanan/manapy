...
def create_info_2dfaces(cellid:'int32[:,:]', nodeid:'int32[:,:]', namen:'uint32[:]', vertex:'float[:,:]', 
                        ...
                        centerf:'float[:,:]', namef:'uint32[:]', tangentf:'float[:,:]'):
    
    norm   = np.zeros(3, dtype=np.float64)
    snorm  = np.zeros(3, dtype=np.float64)
    tang = np.zeros(3,dtype=np.float64)
    ...
    for i in range(nbfaces):
        ...
        # Optionnel : Normalisation de la tangente
        tang[0] =vertex[nodeid[i][1]][0] - vertex[nodeid[i][0]][0] 
        tang[1]=vertex[nodeid[i][1]][1] - vertex[nodeid[i][0]][1] 
        tangentf[i][:]=tang[:]
