...
#        self.comm.Barrier()
    ...
    def compute_cell_gradient(self):
       ...
       def cell_gradient_2d(w_c:'float[:]',nodeid:'float[:, :]',nbnode:'int32',nbcell:'int32',cellidn:'int32[:,:]',nodeidc:'int32[:,:]',nodeidf:'int32[:,:]', faceidc:'int32[:,:]',cellidf:'int32[:,:]',nodeidg:'int32[:,:]', center:'float[:,:]' , nbfaces:'int32', w_x:'float[:]', w_y:'float[:]', w_z:'float[:]'):
        ...
        Variable._cell_gradient(self.cell,self.domain.faces.nodeid,self.domain.nbnodes, self.domain.nbcells,self.domain.nodes.cellid,self.domain.cells.cellnid,self.domain.cells.cellid,
                            self.domain.faces.nodeid,self.domain.nodes.cellid,
                            self.domain.nodes.ghostid,self.domain.cells.center, self.domain.nbfaces, self.gradcellx, 
                            self.gradcelly, self.gradcellz)
