...
class Domain():
    ...
    def __init__(self, dim=None, comm=None, conf=None, **kwargs):
        ...
        self._compute_lf_volums()

        
    def _compute_lf_volums(self):
        nodeid = self._faces.nodeid
        vertex = self._nodes.vertex
        center = self._cells.center
        cellidf= self._faces.cellid, 
        mesuref = self._faces.mesure

        cellidf = cellidf[0] # TODO: because tutple found. need to check

        lf_volums = np.zeros((self._nbfaces, 2))    
        for i in np.concatenate((self._innerfaces, self._halofaces, self._boundaryfaces), axis=0):
            mesure   = mesuref[i]
        ...
        self._faces._lf_volums = lf_volums
    ...
    def _define_face_info(self):
        ...
        if self._dim == 2:
            self.backend.compile(create_info_2dfaces, signature=self.signature)(self._faces._cellid, self._faces._nodeid, self._nodes._name, \
                                ...
                                self._faces._center, self._faces._name, self._faces._tangent)
        elif self._dim == 3:
            self._faces._tangent = np.zeros((self._nbfaces, 3), dtype=self.float_precision)
    ...
    def save_on_cell(self, dt=0, time=0, niter=0, miter=0,RANK=0, value=None):
        ...
        if self.comm.rank == 0:
            ...
            print("Iteration = ", niter, "time = ", time, "time step = ", dt,"Process=", RANK)
    ...
    def save_on_node(self, dt=0, time=0, niter=0, miter=0, RANK=0,value=None):
    ...
    def save_on_node_multi(self, dt=0, time=0, niter=0, miter=0,RANK=0, variables=None, values=None, file_format="vtu"):
    ...
    def save_on_cell_multi(self, dt=0, time=0, niter=0, miter=0,RANK=0, variables=None, values=None, file_format="vtu"):
  ...
  #####ajouter par hanan
    def get_bottom_faces(self):
        return self._bottomfaces  
    def get_upper_faces(self):
        return self._upperfaces  
    def get_out_faces(self):
        return self._outfaces  
    def get_in_faces(self):
        return self._infaces  
