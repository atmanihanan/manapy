...
if RANK == 0:
    ...
    def plot_isovaleurs(Orthocentre, values, title, filename, cmap="rainbow"):
        ...
        # Améliorer l’échelle pour éviter une saturation des couleurs
    ...
    Orthocentre = np.array(cells.center[:, :2])  # Coordonnées des centres des cellules
    rho_vals = np.array(rho.cell)  # Densité
    P_vals = np.array(P.cell)  # Pression
