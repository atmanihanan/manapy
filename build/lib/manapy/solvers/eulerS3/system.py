...
class EulerSolver():
       ...
       # self._update_new_value = self.backend.compile(update_SW, signature=self.signature)
        ...
        if self.RANK==0:
            ...
            while scheme not in self.available_schemes.keys():
                print(">> Wrong choice")
                scheme = input('Choose the scheme (integer): ')
            self.scheme_to_exec = int(scheme)
            sendbuf=int(scheme)
        else:
          sendbuf = None
    ...
    def explicit_convective(self):
        if self.order == 2:
            self.rho.compute_cell_gradient()
            self.rhou.compute_cell_gradient()
            self.rhov.compute_cell_gradient()
            self.rhoE.compute_cell_gradient()
            self.P.compute_cell_gradient()
        # computing the flux
        explicitscheme_convective_euler(self.rho.convective, self.rhou.convective, self.rhov.convective, self.rhoE.convective, 
                                     ...
                                     self.order, self.scheme_to_exec, self.domain.faces.lf_volums)
