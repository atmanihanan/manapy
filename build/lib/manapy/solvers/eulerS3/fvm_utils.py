...
def update_euler(rho_c:'float[:]', rhou_c:'float[:]', rhov_c:'float[:]', rhoE_c:'float[:]',e_interanl_c:'float[:]', P_c:'float [:]',
              rez_rho:'float[:]', rez_rhou:'float[:]', rez_rhov:'float[:]', rez_rhoE:'float[:]',
              dtime:'float', vol:'float[:]',gamma:'float'):

    for i in range(len(rho_c)):
        rho_c[i]   = rho_c[i] + dtime  * rez_rho[i]/vol[i]
        rhou_c[i]  = rhou_c[i]+ dtime  * rez_rhou[i]/vol[i]
        rhov_c[i]  = rhov_c[i]+ dtime  * rez_rhov[i]/vol[i]
        rhoE_c[i]  = rhoE_c[i] +dtime  * rez_rhoE[i]/vol[i]
...
def explicitscheme_convective_euler(rez_rho:'float[:]', rez_rhou:'float[:]', rez_rhov:'float[:]', rez_rhoE:'float[:]', 
                                 ...
                                 P_x:'float[:]', P_y:'float[:]', Px_halo:'float[:]', Py_halo:'float[:]',
                                 psi_rho:'float[:]', psirho_halo:'float[:]',psi_rhou:'float[:]', psirhou_halo:'float[:]',
                                 psi_rhov:'float[:]', psirhov_halo:'float[:]',psi_rhoE:'float[:]', psirhoE_halo:'float[:]',
                                 psi_P:'float[:]', psiP_halo:'float[:]',
                                 ...
                                 innerfaces:'int32[:]', halofaces:'int32[:]', boundaryfaces:'int32[:]', center:'float[:,:]', nodeid:'int32[:,:]',  vertex:'float[:,:]',dt:'float', gamma:'float', order:'int32', scheme_to_exec: 'int32',lf_volums:'float[:,:]'):

    #print("le nombre d'element au bord:",boundaryfaces)
    ...
    for i in innerfaces:
      ...
      # rho
        rho_x_left = rho_x[cellidf[i][0]];  rho_x_right = rho_x[cellidf[i][1]]
        rho_y_left = rho_y[cellidf[i][0]];  rho_y_right = rho_x[cellidf[i][1]]
        
        psirho_left  = psi_rho[cellidf[i][0]];   psirho_right  = psi_rho[cellidf[i][1]]
        ...
        rho_l  = rho_l  + (order - 1) * psirho_left  * (rho_x_left   * r_l[0] + rho_y_left   * r_l[1])
        rho_r  = rho_r  + (order - 1) * psirho_right * (rho_x_right  * r_r[0] + rho_y_right  * r_r[1])
      
      # rhou
        rhou_x_left = rhou_x[cellidf[i][0]];  rhou_x_right = rhou_x[cellidf[i][1]]
        rhou_y_left = rhou_y[cellidf[i][0]];  rhou_y_right = rhou_x[cellidf[i][1]]
        
        psirhou_left  = psi_rhou[cellidf[i][0]];   psirhou_right  = psi_rhou[cellidf[i][1]]
        
        rhou_l  = rhou_l  + (order - 1) * psirhou_left  * (rhou_x_left   * r_l[0] + rhou_y_left   * r_l[1])
        rhou_r  = rhou_r  + (order - 1) * psirhou_right * (rhou_x_right  * r_r[0] + rhou_y_right  * r_r[1])

        #rhov

        rhov_x_left = rhov_x[cellidf[i][0]];  rhov_x_right =  rhov_x[cellidf[i][1]]
        rhov_y_left = rhov_y[cellidf[i][0]];  rhov_y_right = rhov_y[cellidf[i][1]]
        
        psirhov_left  = psi_rhov[cellidf[i][0]];   psirhov_right  = psi_rhov[cellidf[i][1]]
        
        rhov_l  = rhov_l  + (order - 1) * psirhov_left  * (rhov_x_left   * r_l[0] + rhov_y_left   * r_l[1])
        rhov_r  = rhov_r  + (order - 1) * psirhov_right * (rhov_x_right  * r_r[0] + rhov_y_right  * r_r[1])
       # rhoE
        rhoE_x_left = rhoE_x[cellidf[i][0]];  rhoE_x_right = rhoE_x[cellidf[i][1]]
        rhoE_y_left = rhoE_y[cellidf[i][0]];  rhoE_y_right = rhoE_y[cellidf[i][1]]
        
        psirhoE_left  = psi_rhoE[cellidf[i][0]];   psirhoE_right  = psi_rhoE[cellidf[i][1]]
        
        rhoE_l  = rhoE_l  + (order - 1) * psirhoE_left  * (rhoE_x_left   * r_l[0] + rhoE_y_left   * r_l[1])
        rhoE_r  = rhoE_r  + (order - 1) * psirhoE_right * (rhoE_x_right  * r_r[0] + rhoE_y_right  * r_r[1])

        # p
        P_x_left = P_x[cellidf[i][0]];  P_x_right = P_x[cellidf[i][1]]
        P_y_left = P_y[cellidf[i][0]];  P_y_right = P_y[cellidf[i][1]]
        
        psiP_left  = psi_P[cellidf[i][0]];   psiP_right  = psi_P[cellidf[i][1]]
        
        P_l  = P_l  + (order - 1) * psiP_left * (P_x_left   * r_l[0] + P_y_left   * r_l[1])
        P_r  = P_r  + (order - 1) * psiP_right * (P_x_right  * r_r[0] + P_y_right  * r_r[1])


   
        # exec method
        if scheme_to_exec == 0: #"rusanov"
          ...
          Roe_version1(rho_l,rhou_l,rhov_l, rhoE_l,P_l,rho_r,rhou_r, rhov_r,rhoE_r,P_r ,gamma, normal, tangent,flux,mesure)
    ...
    for i in halofaces:
      ...
      ####ordre2####
        ...
        r_l[1] = centerf[i][1] - center_left[1]; r_r[1] = centerf[i][1] - center_right[1]; 
      # rho
        rho_x_left = rho_x[cellidf[i][0]];  rho_x_right = rhox_halo[halofid[i]]
        rho_y_left = rho_y[cellidf[i][0]];  rho_y_right = rhoy_halo[halofid[i]]
        
        psirho_left  = psi_rho[cellidf[i][0]];   psirho_right  = psirho_halo[halofid[i]]
      ...
      # rhou
        ...
        psirhou_left  = psi_rhou[cellidf[i][0]];   psirhou_right  = psirhou_halo[halofid[i]]
        ...
        psirhov_left  = psi_rhov[cellidf[i][0]];   psirhov_right  = psirhov_halo[halofid[i]]
       ...
       # rhoE
        ...
        psirhoE_left  = psi_rhoE[cellidf[i][0]];   psirhoE_right  = psirhoE_halo[halofid[i]]
        ...
        psiP_left  = psi_P[cellidf[i][0]];   psiP_right  = psiP_halo[halofid[i]]
    ...
    for i in boundaryfaces:
      ...
      # rho
        ...
        psirho_left  = psi_rho[cellidf[i][0]]
        ...
        psirhou_left  = psi_rhou[cellidf[i][0]]
        ...
        psirhov_left  = psi_rhov[cellidf[i][0]]
       ...
       # rhoE
        ...
        psirhoE_left  = psi_rhoE[cellidf[i][0]]
